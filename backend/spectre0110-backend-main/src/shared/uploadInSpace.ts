...
const DO_CONFIG = {
  ...
  region: config.doSpaces.region as string,
  credentials: {
    accessKeyId: config.doSpaces.accessKeyId as string,
    secretAccessKey: config.doSpaces.secretAccessKey as string,
  },
  spaceName: config.doSpaces.bucket as string,
};

if (
  !config.doSpaces.endpoint ||
  !config.doSpaces.accessKeyId ||
  !config.doSpaces.secretAccessKey ||
  !config.doSpaces.bucket
) {
  throw new Error(
    "Missing DigitalOcean Spaces configuration. Set DO_SPACES_ENDPOINT, DO_SPACES_REGION, DO_SPACES_ACCESS_KEY_ID, DO_SPACES_SECRET_ACCESS_KEY and DO_SPACES_BUCKET in .env"
  );
}
