...
**Native SwiftUI mapping**: this maps almost 1:1 onto `.spring(response:dampingFraction:)` / `.animation(.spring(response:dampingFraction:), value:)` — SwiftUI uses the same "response" and "dampingFraction" vocabulary Apple standardized on, so the concrete values in this skill (damping ratio, response in seconds) can be used directly, not just translated.
...
Apple uses translucent materials as a floating functional layer that brings structure without stealing focus. On the web, approximate with `backdrop-filter`. Natively, this is `.ultraThinMaterial` / `.regularMaterial` / `.thickMaterial` in SwiftUI, and the same hierarchy rules below apply directly.
...
2. **Harmony** — the visual, the sound, and the haptic must fire on the **same frame**. Latency between them destroys the illusion. Don't let a CSS transition lag the audio/haptic (Vibration API). Natively, pair with `UIImpactFeedbackGenerator`/`UINotificationFeedbackGenerator` (or SwiftUI's `.sensoryFeedback(_:trigger:)`) fired from the same state change that drives the animation, not a delayed callback.
...
- **`prefers-reduced-motion: reduce`** (native: `UIAccessibility.isReduceMotionEnabled` / SwiftUI `@Environment(\.accessibilityReduceMotion)`) — replace slides/springs/parallax with short opacity **cross-fades or static transitions**. Drop elastic/overshoot. Keep opacity/color changes that aid comprehension.
- **`prefers-reduced-transparency: reduce`** (native: `UIAccessibility.isReduceTransparencyEnabled`) — make translucent surfaces frostier/solid: raise background opacity, drop the blur.
...
- **Respect the user's text-size setting** (Dynamic Type). Scale layout *with* the text — spacing in `rem`/`em` on the web, or SwiftUI's relative `.font(.body)`-style text styles rather than fixed point sizes — so a larger font doesn't break the layout.
